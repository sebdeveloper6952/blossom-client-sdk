...
export class HTTPError extends Error {
    ...
    static async handleErrorResponse(res: Response) {
        if (!res.ok) {
            try {
                const uploadMsg = res.headers.get("X-Upload-Message");
                if (uploadMsg !== null) {
                    throw new HTTPError(res, uploadMsg);
                }
...
export class BlossomClient {
    ...
    static async uploadRequirements(server: ServerType, file: UploadType, auth?: SignedEvent) {
        const sha256 = await BlossomClient.getFileSha256(file);
        let contentLength = 0;
        let contentType = "";
        ...
        if (auth) {
            headers.set("Authorization", BlossomClient.encodeAuthorizationHeader(auth));
        }
        headers.set("X-SHA-256", sha256);
        headers.set("X-Content-Length", contentLength.toString());
    ...
    async uploadRequirements(file: UploadType, auth: SignedEvent | boolean = true) {
        if (typeof auth === "boolean" && auth) auth = await this.getUploadAuth(file);
        return BlossomClient.uploadRequirements(this.server, file, auth ? auth : undefined);
    }
